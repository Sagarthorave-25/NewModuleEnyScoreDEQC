...
namespace ENYSchedular_new
{
    class helper
    {
        ...
        public DataSet GetEmptyEyData()
        {
            ...
            {
                new CommFun().SaveEYRiskScoreErrorLog("GetApplicationEy", "GetEmptyEyData", "helper", ex.Message, "Error", "NewEYModelShedular_DEQC");
        ...
        public void GenerateEyScore_Saral(DataSet dst)
        {
            ...
            string Source = "NewEYModelShedular_DEQC";
                ...
                if (dst.Tables.Count > 0 && dst.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < dst.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(ApplicationNo))
                        {
                            ...
                            {
                                new CommFun().SaveEYRiskScoreErrorLog(ApplicationNo, "NewEYModelShedular_DEQC", "helper", "Application Value not present in database", "Error", Source);
                            }
                        }
                    }
                }
                else {
                    new CommFun().SaveEYRiskScoreErrorLog(ApplicationNo, "NewEYModelShedular_DEQC", "helper", "No application present in database", "Error", Source);
                }
            }
            catch (Exception ex)
            {
                string msg = ex.Message;
                new CommFun().SaveEYRiskScoreInputOutputLog(inputJson, result, ApplicationNo, Source, result, "Update", SrNo);
                new CommFun().SaveEYRiskScoreErrorLog(ApplicationNo, "GenerateEyScore_DEQC", "helper", ex.Message, "Error", Source);
        ...
        public  string SmartApi(string apiUrl, string inputJson, string ApplicationNo,  string Source)
        {
